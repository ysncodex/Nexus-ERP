...
const OrderProductCard = memo(function OrderProductCard({ item, qty, onAdd }: { item: MenuItem; qty: number; onAdd: (i: MenuItem) => void }) {
  return (
    <button
      type="button"
      onClick={() => onAdd(item)}
      ...
      <div className="bg-slate-50 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full max-w-md max-h-[95vh] sm:max-h-[92vh] flex flex-col overflow-hidden border border-slate-200/80">
        ...
        <div className="p-4 border-t border-slate-200 shrink-0 space-y-2.5 bg-white">
          {activeTab !== 'payment' && (
            ...
            >
              {primaryPrinting ? <Loader2 size={16} className="animate-spin" /> : <Printer size={16} />}
              {primaryLabel}
            </button>
          )}
          {activeTab === 'payment' ? (
          ...
          <p className="text-[10px] text-center text-slate-400 leading-relaxed px-2">{footerHint}</p>
        </div>
      </div>
    </div>
  );
...
export default function NewOrder() {
  ...
  }, [canMutate, orderItems, customerName, tableNumber, paymentMethod, channel, discountType, discountValue]);
  ...
  const handlePrintAndComplete = useCallback(
    async (order: NewOrderData, kind: 'customer' | 'both') => {
      ...
      toast.success(orderSavedMessage(order, 'completed', result), {
        description:
          result === 'queued'
            ? `${getOfflineQueueCount()} receipt(s) waiting to sync`
            : undefined,
      });
      resetPos();
      void kind;
  ...
  return (
    ...
    <div className="flex flex-col xl:flex-row gap-4 flex-1 min-h-0">
      ...
      <div className="flex-[3] min-w-0 flex flex-col gap-3 overflow-hidden min-h-[280px] xl:min-h-0">
      ...
      {/* ── RIGHT: Order Cart (40%) ── */}
      <div className="w-full xl:flex-[2] xl:min-w-[320px] shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm flex flex-col overflow-hidden min-h-[360px] max-h-[70dvh] xl:max-h-none xl:min-h-0">
        ...
        <div className="border-t border-slate-100 p-3.5 space-y-3 shrink-0">
          ...
          {!canMutate && (
            <p className="text-[11px] text-center text-slate-400 mt-2 leading-relaxed">
              You can build a cart to explore the flow. Sign in as Owner or Manager to complete orders.
            </p>
          )}
        </div>
      </div>
